...
FrameBuffer::FrameBuffer(Texture tex){
  ...
  if(status != GL_FRAMEBUFFER_COMPLETE){
    printf("Error making framebuffer %x\n", glGetError());
  }
...
FrameBuffer::FrameBuffer(int width,int height,int colorChannels, int interp_param, int wrap_param){
	...
	if(status != GL_FRAMEBUFFER_COMPLETE){
	  printf("1:  Error making framebuffer status: %x error:%x\n", status ,glGetError());
	...
	glFramebufferRenderbuffer(GL_FRAMEBUFFER,GL_DEPTH_ATTACHMENT,GL_RENDERBUFFER, stencil_buffer);	
	glFramebufferRenderbuffer(GL_FRAMEBUFFER,GL_STENCIL_ATTACHMENT,GL_RENDERBUFFER, stencil_buffer);
	status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
 	if(status != GL_FRAMEBUFFER_COMPLETE){
	  printf("2:  Error making framebuffer status: %x error:%x\n", status ,glGetError());
